...
class CpaProcessBudget extends CpaProcessProduct
{
    ...
    public function __construct($id_product, $datacustom)
    {
        ...
        $this->moduletrans = Module::getInstanceByName('cpacustomizadorprodutosaluclass');
    ...
    private function generatePDF($pricewithreduction, $pricewithreductiontax, $link, $token)
    {
        ...
        $pdf->SetTitle($this->moduletrans->l('Orçamento', 'CpaProcessBudget'));
        ...
        $pdf->Cell(0, 10, $this->moduletrans->l('ORÇAMENTO', 'CpaProcessBudget'), 0, 1, 'C');
        $pdf->Ln(5);
        ...
        $html = '<a href="' . $link . '" target="_blank" >'.$this->moduletrans->l('Editar/Adicionar personalização', 'CpaProcessBudget').'</a><br><br>';
        $html .= '<table border="1" cellpadding="5">
                        <tr>
                            <th width="30%"><strong>' . $this->moduletrans->l('Imagem', 'CpaProcessBudget') . '</strong></th>
                            <th width="70%"><strong>' . $this->moduletrans->l('Descrição', 'CpaProcessBudget') . '</strong></th>
                        </tr>
        ...
        $pdf->Ln(5);
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(180, 10, sprintf($this->moduletrans->l('Total SEM IVA : %s €', 'CpaProcessBudget'),round($pricewithreductiontax, 2)), 0, 1, 'R');
        $pdf->Cell(180, 10, sprintf($this->moduletrans->l('IVA : %s €', 'CpaProcessBudget'),round($pricewithreduction - $pricewithreductiontax, 2)), 0, 1, 'R');

        $pdf->SetFont('helvetica', 'B', 12);
        $pdf->Cell(180, 10, sprintf($this->moduletrans->l('Total COM IVA : %s €','CpaProcessBudget'),round($pricewithreduction, 2)), 0, 1, 'R');
