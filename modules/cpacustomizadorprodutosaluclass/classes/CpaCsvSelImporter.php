...
class CpaCsvSelImporter
{
    ...
    public function importCSV($filePath)
    {
        ...
        //     $this->errors[] = $translator->l('CSV seleção inválido', 'CpaCsvSelImporter');
        //     return false;
        // }

        if (!file_exists($filePath)) {
            $this->errors[] = $translator->l('Ficheiro não encontrado', 'CpaCsvSelImporter');
            return false;
        }
        ...
        if ($handle === false) {
            $this->errors[] = $translator->l('Erro ao abrir o ficheiro CSV seleção', 'CpaCsvSelImporter');
            return false;
        }

        if (!$this->deleteCSV()) {
            $this->errors[] = $translator->l('Erro ao deletar registos na base de dados', 'CpaCsvSelImporter');
            return false;
        }
                ...
                foreach ($this->arraywidth as $key => $width) {
                    ...
                    if (is_numeric(str_replace(',', '.', $arrayprice[$key]))) {
                        ...
                        if (!$this->SetRowCSV($price, $width)) {
                            $this->errors[] = $translator->l('Erro ao inserir na base de dados', 'CpaCsvSelImporter');
                        }
                    }
                }
            }
